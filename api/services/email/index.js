...
const EMAIL_DISABLE_ALL = String(process.env.EMAIL_DISABLE_ALL || '').toLowerCase() === 'true';
...
const MAX_TOTAL_SIZE_BYTES = MAX_TOTAL_SIZE_MB > 0 ? MAX_TOTAL_SIZE_MB * 1024 * 1024 : 0;
...
async function sendRaw({ to, subject, html, text, attachments }) {
  ...
  if (MAX_TOTAL_SIZE_BYTES) {
    ...
    if (total > MAX_TOTAL_SIZE_BYTES) {
      const err = new Error(`EMAIL_TOO_LARGE: ${total} bytes > ${MAX_TOTAL_SIZE_BYTES}`);
  ...
  if (!allowed) {
    ...
    console.log('[EMAIL SANDBOX] BLOCKED send to=%o subject="%s"', toList, subject);
    return { id: 'sandbox-skip' };
  }

  // Delegamos en el adaptador del proveedor (Resend)
  return resendSend({ from: FROM, to: toList, subject, html, text, attachments });
}
  ...
  async sendTemplate({ template, to, data }) {
    ...
    switch (template) {
      ...
      case 'patient_welcome': {
        ...
        const { patientWelcomeTemplate } = await import('./templates/patientWelcome.js');
        const t = patientWelcomeTemplate({ ...data });
        return sendRaw({ to, ...t });
      }
      // Cita creada
      case 'appointment_booked': {
        const { appointmentBookedTemplate } = await import('./templates/appointmentBooked.js');
      ...
      case 'professional_welcome': {
        const { professionalWelcomeTemplate } = await import('./templates/professionalWelcome.js');
