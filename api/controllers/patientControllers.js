...
export const postNewPatient = async (req, res) => {
  ...
  if (typeof postalCode !== 'string' || !/^\d{5}$/.test(postalCode.trim())) {
    validationErrors.push({
      field: 'postalCode',
      code: VALIDATION_CODES.POSTAL_CODE_INVALID_FORMAT,
    });
  }
  ...
  if (!/^\+?\d{7,15}$/.test(emergencyContact)) {
    validationErrors.push({
      ...
      code: VALIDATION_CODES.EMERGENCY_CONTACT_INVALID,
    });
  }
...
export const putEditPatient = async (req, res) => {
  try {
    ...
    if (!birthDate) {
      validationErrors.push({
        field: 'birthDate',
        code: VALIDATION_CODES.BIRTHDATE_REQUIRED || 'BIRTHDATE_REQUIRED',
      });
    } else {
    ...
    if (phoneExists) {
      return validationError(
        res,
        [{ field: 'phone', code: VALIDATION_CODES.PHONE_ALREADY_EXISTS }],
        409
      );
    }
